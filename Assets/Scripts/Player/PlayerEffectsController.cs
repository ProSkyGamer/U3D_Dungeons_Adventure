...
public class PlayerEffectsController : NetworkBehaviour
{
    ...
    public enum AllPlayerEffects
    {
        ...
        GainedCoinsOnEnemyDeathIncrease,
        StaminaConsumptionDecrease,
        SkillPointExpRequirementDecrease, //NOT ADDED
        PlayerInventorySlotsIncrease,
        RelicInventorySlotsIncrease,
        WeaponInventorySlotsIncrease,
        OnHitEnemySpeedDecrease,
    ...
    private int GetEffectAppliedTimesByConditions(AppliedEffect appliedEffect)
    {
        ...
        switch (effectCondition.conditionType)
        {
            case AppliedEffect.EffectCondition.ConditionType.CurrentHpHigherThen:
                maxValue = playerHealthController.GetCurrentMaxHp();
                currentValue = playerHealthController.GetCurrentHealth();
                isHigherThen = true;
                break;
            ...
            case AppliedEffect.EffectCondition.ConditionType.DefHigherThen:
                maxValue = playerHealthController.GetCurrentDefence();
    ...
    private void AddOrRemoveEffectToController(AppliedEffect appliedEffect, bool isAdding = true)
    {
        ...
        switch (applyingEffectType)
        {
            ...
            case AllPlayerEffects.DefIncrease:
                playerHealthController.ChangeDefenceBuff(effectPercentageValue, effectFlatValue);
                break;
            ...
            case AllPlayerEffects.TakenDmgDecrease:
                playerHealthController.ChangeTakenDamageAbsorptionBuff(effectPercentageValue, effectID);
                if (isLimited)
                    if (!IsAlreadyHasCurrentLimitedBuff(applyingEffectType))
                        if (isAdding)
                            playerHealthController.OnHealthAbsorptionTriggered +=
            ...
            case AllPlayerEffects.TakenDmgIncrease:
                playerHealthController.ChangeTakenDamageIncreaseDebuff(effectPercentageValue);
                break;
            ...
            case AllPlayerEffects.DeathSaving:
                playerHealthController.ChangeDeathSavingEffect(effectPercentageValue, effectID);
                if (isLimited)
                    if (!IsAlreadyHasCurrentLimitedBuff(applyingEffectType))
                        if (isAdding)
                            playerHealthController.OnDeathSavingEffectTriggered +=
                                BuffedController_OnLimitedBuffEffectTriggered;
                        else
                            playerHealthController.OnDeathSavingEffectTriggered -=
                                BuffedController_OnLimitedBuffEffectTriggered;
